...
#else
#define EXPRESSION_STYLE    (QString("QLabel { font-size: 30pt; }"))
#define READING_STYLE       (QString("QLabel { font-size: 25pt; }"))
#endif
...
//    m_ui->buttonCollapse->hide();
    QPixmap immersionIcon(DirectoryUtils::getConfigDir() + "png/immersionKit.png");
    m_ui->buttonCollapse->setIcon(immersionIcon);
    ...
    m_ui->buttonAddCard->setIcon(QPixmap(DirectoryUtils::getConfigDir() + "png/term_add.png"));
    m_ui->buttonAddCard->setVisible(false);

    m_ui->buttonKanaKanji->setVisible(false);

    m_ui->buttonAnkiOpen->setIcon(QPixmap(DirectoryUtils::getConfigDir() + "png/share.png"));
    m_ui->buttonAnkiOpen->setVisible(false);

    m_ui->buttonAudio->setIcon(QPixmap(DirectoryUtils::getConfigDir() + "png/term_speaker.png"));
    ...
    connect(
        ...
        this, &TermWidget::searchImmersionKit
    );
    connect(
    ...
    connect(
            m_ui->labelKana, &QLabel::linkActivated,
            this, &TermWidget::kanaClicked
            );
    connect(
    ...
    const Term &term, Qt::KeyboardModifier modifier, GlossaryStyle style)
{
    ...
    m_ui->labelKana->setText(QString("<style> a { text-decoration: none;}</style><a href=\" \">" + term.reading + "</a>"));
    m_ui->labelKana->setFocusPolicy(Qt::NoFocus);
...
//    term->sentence = player->getSubtitle(true);
    term->sentence = GlobalMediator::getGlobalMediator()->getTextHolder()->getSentence();
...
void TermWidget::setAddable(bool expression, bool reading)
{
    ...
    {
        m_ui->buttonAddCard->setVisible(true);
    }
...
//    );
//    if (m_ui->buttonAnkiOpen->isVisible()) m_ui->buttonAddCard->hide();
...
void TermWidget::kanaClicked()
{
    QString str = m_ui->labelKana->text();
    str.remove(0,55);
    str.chop(4);
    Q_EMIT GlobalMediator::getGlobalMediator()->sendKanaClicked(str);
}

void TermWidget::searchImmersionKit()
{
    QString urlbase("https://www.immersionkit.com/dictionary?keyword=");
    QDesktopServices::openUrl(QUrl(urlbase + m_term->expression));
}
