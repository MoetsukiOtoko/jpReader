...
bool AnkiClient::readConfigFromFile(const QString &filename)
{
    ...
    for (const QJsonValue &val : profiles)
    {
        ...
        for (const QJsonValue &v : profile[CONFIG_EXCLUDE_GLOSSARY].toArray())
        {
            if (val.isString())
            {
...
AnkiReply *AnkiClient::notesAddable(QList<QSharedPointer<const Term>> terms)
{
    ...
    [this, terms, ankiReply]
    {
        ...
        for (QSharedPointer<const Term> term : terms)
        {
            ...
            notes.append(createAnkiNoteObject(termCopy));
...
AnkiReply *AnkiClient::notesAddable(QList<QSharedPointer<const Kanji>> kanji)
{
    ...
    QThreadPool::globalInstance()->start(
        [this, kanji, ankiReply] {
            QJsonArray notes;
            for (QSharedPointer<const Kanji> k : kanji)
            {
                notes.append(createAnkiNoteObject(*k));
            }
            QJsonObject params;
            params[ANKI_CAN_ADD_NOTES_PARAM] = notes;
            Q_EMIT sendBoolListRequest(
...
AnkiReply *AnkiClient::addNote(const Term *term)
{
    ...
    QThreadPool::globalInstance()->start(
        [=] {
            ...
            QJsonObject note = createAnkiNoteObject(*term, filemap);
            if (!filemap.isEmpty())
            {
...
AnkiReply *AnkiClient::addNote(const Kanji *kanji)
{
    ...
    QThreadPool::globalInstance()->start(
        [=] {
            QJsonObject params;
            QJsonObject note = createAnkiNoteObject(*kanji);
            delete kanji;
            params[ANKI_PARAM_ADD_NOTE] = note;
            Q_EMIT sendIntRequest(ANKI_ACTION_ADD_NOTE, params, ankiReply);
        }
...
{
    QList<QPair<QString, QString>> filemap;
    return createAnkiNoteObject(term, filemap);
}
    ...
    QList<QPair<QString, QString>> &filemap)
{
    ...
    buildCommonNote(
        m_currentConfig->termFields, term, note, fieldsObj
    );
...
QJsonObject AnkiClient::createAnkiNoteObject(const Kanji &kanji)
{
    ...
    QJsonObject fieldsObj;
    buildCommonNote(
        m_currentConfig->kanjiFields, kanji, note, fieldsObj
    );
